...
const getAllAdmins = async (req, res) => {
  const result = await admins.findAll({ attributes: ['id', 'userName'] });
...
const adminReg = async (req, res) => {
  ...
  const isAlreadyExist = await admins.findOne({
  ...
  await admins.create({ userName: userName, password: hassedPass });
  res
